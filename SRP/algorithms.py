...
def division_error(individuals, points):
    division_count = 0
...
def division_count(individuals):
    division_count = 0

    for individual in individuals:
        if "/" in individual.phenotype:
            division_count +=1
...
def ge_eaSimpleWithElitism(population, toolbox, cxpb, mutpb, ngen, elite_size, 
    ...
    div_count = division_count(valid0)
    div_error = division_error(valid0, points_train)
    ...
    for gen in range(logbook.select("gen")[-1]+1, ngen + 1):
        ...
        if points_test: 
            ...
            logbook.record(gen=gen, invalid=invalid, **record, 
                       ...
                       selection_time=selection_time,
                       generation_time=generation_time,
                       division_errors=div_error, division_count = div_count)
