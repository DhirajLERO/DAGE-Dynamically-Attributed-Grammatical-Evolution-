...
def finalize_division_int(phenotype, idx_genome, genome):
    ...
    if idx_genome is not None:
        if check_zero_denominator(phenotype_temp):
            # consume a codon and find constant
            ...
            return "x[0] + " + replacement, idx_genome_temp - idx_genome, remainders, possible_choices
        else:
            return "x[0]", 0, [], []
    else:
        if check_zero_denominator(phenotype_temp):
            # similarly create a codon and define a constant to add to x[0]
            l = [-5, -4.5, -4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]
            remainder = random.randint(0, len(l) - 1)
            # print("inside sensible initiation", [remainder], [len(l)])
            return "x[0] + " +  str(l[remainder]), 0, [remainder], [len(l)]
        else:
            return "x[0]", 0, [], []
...
import pickle
with open('./results/DAGEV1/testfitness.pkl', 'wb') as fp:
