...
def search_recent_direction(s, substrings):
    last_found = None
    last_index = -1
...
def find_choice(directions, genome, idx_genome):
    ...
    return replacement, idx_genome, [x], [len(directions)]


def finalize_direction(phenotype, idx_genome, genome):
    # print(phenotype)
    directions = [" move_forward ", " turn_left ", " turn_right "]
    idx_genome_temp = copy.deepcopy(idx_genome)
    phenotype_temp = copy.deepcopy(phenotype)
    last_keyword = search_recent_direction(phenotype_temp, directions)

    no_of_codon_used = 0

    if idx_genome is not None:
        if last_keyword is not None:
            directions.remove(last_keyword)

        replacement, idx_genome_temp, remainders, possible_choices = find_choice(directions, genome, idx_genome_temp)
        return replacement, idx_genome_temp - idx_genome, remainders, possible_choices
    else:
        if last_keyword is not None:
            directions.remove(last_keyword)
        remainder = random.randint(0, len(directions) - 1)
        # print("inside sensible initiation", [remainder], [len(l)])
        return directions[remainder], no_of_codon_used, [remainder], [len(directions)]
...
#     if idx_genome is not None:
#         if last_keyword is not None:
#             if last_keyword == " turn_left ":
#                 directions.remove(" turn_right ")
#
#             if last_keyword == " turn_right ":
#                 directions.remove(" turn_left ")
#         replacement, idx_genome_temp, remainders, possible_choices = find_choice(directions, genome, idx_genome_temp)
#         return replacement, idx_genome_temp - idx_genome, remainders, possible_choices
#     else:
#         if last_keyword is not None:
...
function_dict = {"finalize_direction": finalize_direction}
...
for i in range(N_RUNS):
  ...
  # We set up this inside the loop for the case in which the data is defined randomly
    ...
    with open(r"./results/DAGE/" + str(r) + ".csv", "w", encoding='UTF8', newline='') as csvfile:
...
with open('./results/DAGE/max_moves.pkl', 'wb') as fp:
    pickle.dump(move_count, fp)

with open('./results/DAGE/best_phenotype_each_run.pkl', 'wb') as fp:
